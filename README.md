...
## DAREBEE 30 Days of HIIT companion

The Today screen now links to a 30-day companion tracker. Each day opens the verified official workout URL (`?start=1` through `?start=30`), and the video button opens DAREBEE's official exercise library. Track the level actually completed, minutes, effort and notes. A completion contributes one session / 100 XP; editing it never duplicates the reward. Undo removes the completion while keeping personal recordings. Your recordings can be attached to each day and reviewed in the existing video library. Existing gym sessions, weights and progress are preserved.

The program follows its official duration; the 15–90 minute gym budget does not rescale HIIT intervals. Day advancement is chosen by the user, not enforced by the calendar. There is no automatic claim that the official workout was performed—completion is explicitly self-reported.

DAREBEE's [Terms of Use](https://darebee.com/terms-of-use.html) explicitly prohibit inclusion of its materials in packaged products, including free mobile apps. Consequently this implementation includes source links and independent progress tracking, not copies of DAREBEE animations, cards, program instructions or branding artwork. External workouts/videos require an internet connection; local logs remain available offline. Embedding/repackaging DAREBEE assets requires separate permission. SHOW UP is not affiliated with DAREBEE.
